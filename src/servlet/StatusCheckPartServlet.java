			...
			if(page.equals("DefinitiveRegistration")) {
				...
				if(Status[5].equals("食材不足")) {
					...
					if(Status[4].equals("R3") && Status[3].equals("6")) {
						...
						result = RoundsDAO.UpdateTour(user_num , "TOUR_FIN" ,tour_fin ) ;

						//R3のH6 ⇒ツアーテーブル挿入 ToursDAO.AddTour()
						result = ToursDAO.AddRound1_AddTour(user_num);
						...
						result = RoundsDAO.AddRoundUpdateTour(user_num , Status[4] ,tour_num );

					}else {
						int round_num = Integer.parseInt(Status[2]);
						int hole_num = Integer.parseInt(Status[3]);
						result = RoundsDAO.AddHole(user_num ,round_num ,hole_num);
					...
					if(result == true) {
						...
						Status = StatusDAO.getStatus(user_num);
